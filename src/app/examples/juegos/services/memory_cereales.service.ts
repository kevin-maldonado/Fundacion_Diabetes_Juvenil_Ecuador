...
import { TileImage3 } from '../Memory_cereales/interfaces3/tile-image3';
import { levelNames3, levels3, tiles3 } from '../Memory_cereales/enums3/enums3';

@Injectable({
  providedIn: 'root'
})
export class MemoryCerealesService {
  ...
  private level$: levels3 = levels3.easy;
  private tiles$: Array<TileImage3>;
  private gameTilesKeys$: Array<string>;
  private selectedTile1$: TileImage3;
  private selectedTile2$: TileImage3;
  ...
  set level(value: levels3) {
    this.level$ = value;
  }

  get levelName(): string {
    if (this.level$ === levels3.easy) {
      return levelNames3.easy;
    } else if (this.level$ === levels3.medium) {
      return levelNames3.medium;
    }
    return levelNames3.hard;
  }

  get tiles(): Array<TileImage3> {
    return this.tiles$;
  }
  ...
  private initGameTiles(): void {
    ...
    const tilesKeys: Array<string> = Object.keys(tiles3);
    this.gameTilesKeys$ = [];

    while (this.gameTilesKeys$.length < (Math.pow(levels3.hard, 2) / 2)) {
  ...
  private initGameGrid() {
    ...
    this.tiles$ = Array(this.totalGameTiles$).fill('').map(() => {
      ...
      const tile: TileImage3 = {
        id: tileId,
        path: `./assets/images/cereales_granos/${tiles3[tileId]}`,
        fixed: false,
      };
      return tile;
    });
   ...
   */
  setTurnTile(tile: TileImage3): void {
