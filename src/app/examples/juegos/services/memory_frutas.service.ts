...
import { levelNames1, levels1, tiles1 } from '../Memory_frutas/enums1/enums1';
import { TileImage1 } from '../Memory_frutas/interfaces1/tile-image1';

@Injectable({
  providedIn: 'root'
})
export class MemoryFrutasService {
  ...
  private level$: levels1 = levels1.easy;
  private tiles$: Array<TileImage1>;
  private gameTilesKeys$: Array<string>;
  private selectedTile1$: TileImage1;
  private selectedTile2$: TileImage1;
  ...
  set level(value: levels1) {
    this.level$ = value;
  }

  get levelName(): string {
    if (this.level$ === levels1.easy) {
      return levelNames1.easy;
    } else if (this.level$ === levels1.medium) {
      return levelNames1.medium;
    }
    return levelNames1.hard;
  }

  get tiles(): Array<TileImage1> {
    return this.tiles$;
  }
  ...
  private initGameTiles(): void {
    ...
    const tilesKeys: Array<string> = Object.keys(tiles1);
    this.gameTilesKeys$ = [];

    while (this.gameTilesKeys$.length < (Math.pow(levels1.hard, 2) / 2)) {
  ...
  private initGameGrid() {
    ...
    this.tiles$ = Array(this.totalGameTiles$).fill('').map(() => {
      ...
      const tile: TileImage1 = {
        id: tileId,
        path: `./assets/images/frutas/${tiles1[tileId]}`,
        fixed: false,
      };
      return tile;
    });
   ...
   */
  setTurnTile(tile: TileImage1): void {
