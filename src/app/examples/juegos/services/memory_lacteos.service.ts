...
import { levelNames2, levels2, tiles2 } from '../Memory_lacteos/enums2/enums2';
import { TileImage2 } from '../Memory_lacteos/interfaces2/tile-image2';

@Injectable({
  providedIn: 'root'
})
export class MemoryLacteosService {
  ...
  private level$: levels2 = levels2.easy;
  private tiles$: Array<TileImage2>;
  private gameTilesKeys$: Array<string>;
  private selectedTile1$: TileImage2;
  private selectedTile2$: TileImage2;
  ...
  set level(value: levels2) {
    this.level$ = value;
  }

  get levelName(): string {
    if (this.level$ === levels2.easy) {
      return levelNames2.easy;
    } else if (this.level$ === levels2.medium) {
      return levelNames2.medium;
    }
    return levelNames2.hard;
  }

  get tiles(): Array<TileImage2> {
    return this.tiles$;
  }
  ...
  private initGameTiles(): void {
    ...
    const tilesKeys: Array<string> = Object.keys(tiles2);
    this.gameTilesKeys$ = [];

    while (this.gameTilesKeys$.length < (Math.pow(levels2.hard, 2) / 2)) {
  ...
  private initGameGrid() {
    ...
    this.tiles$ = Array(this.totalGameTiles$).fill('').map(() => {
      ...
      const tile: TileImage2 = {
        id: tileId,
        path: `./assets/images/lacteos/${tiles2[tileId]}`,
        fixed: false,
      };
      return tile;
    });
   ...
   */
  setTurnTile(tile: TileImage2): void {
